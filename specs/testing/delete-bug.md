...
#### 1.2. Clicking Delete opens a confirmation modal before the bug is removed
...
**Steps:**
  ...
  4. Click the Delete button in the edit modal.
    ...
    - expect: The bug is still visible on the board and remains in the database.

#### 1.3. Confirming deletion removes the bug from the board and closes the modal
...
**Steps:**
  ...
  5. Confirm the deletion in the confirmation modal.
    ...
    - expect: The bug is removed from the database.

#### 1.4. Canceling deletion keeps the bug in place
...
**Steps:**
  ...
  4. Click the Delete button in the edit modal.
    - expect: A confirmation modal appears.
  5. Cancel the confirmation modal.
    - expect: The confirmation modal closes.
    - expect: The edit modal remains open.
    - expect: The bug remains visible on the board and in the database.

#### 1.5. Deleting a bug that was created during test setup does not affect other bugs
...
**Steps:**
  ...
  2. Open the edit modal for the freshly created bug and delete it through confirmation.
    - expect: The confirmation modal appears.
    - expect: The bug is removed only after confirmation.
